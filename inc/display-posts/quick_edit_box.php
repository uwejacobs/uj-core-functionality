 ...
 */
if (!function_exists('ujcf_add_quick_edit')) {
    // $column_name stores only the custom fields
    function ujcf_add_quick_edit($column_name, $post_type)
    {
	...
	case 'status':
		...
		$status = $status[0] ?? "";
                ...
                wp_nonce_field('ujcf_q_edit_nonce', 'ujcf_nonce');
    ...
    add_action('quick_edit_custom_box',  'ujcf_add_quick_edit', 10, 2);
}
 ...
 */
if (!function_exists(('ujcf_quick_edit_save'))) {
    function ujcf_quick_edit_save($post_id)
    {
    ...
    add_action('save_post_booking', 'ujcf_quick_edit_save');
}
 ...
 */
if (!function_exists('ujcf_quick_edit_js')) {
    function ujcf_quick_edit_js()
    {
        ...
        ?>
        <!-- add JS script -->
        <script id="ujcf_quick_edit_js" type="text/javascript">
            jQuery(function($) {
...
<?php
    ...
    add_action('admin_print_footer_scripts-edit.php', 'ujcf_quick_edit_js');
}
