...
if (!function_exists('ujcf_add_quick_edit_order')) {
    // $column_name stores only the custom fields
    function ujcf_add_quick_edit_order($column_name, $post_type)
    {
        if ($column_name !== 'order') return;

		$field_name = ujcf_get_quick_edit_field_name($post_type);
		if (!$field_name) return;

		global $post;
		$order = get_post_meta($post->ID, "order");
		$order = $order[0] ?? 10;
		echo '<fieldset class="inline-edit-col-right" style="border: 1px solid #dddddd;">
				<legend style="font-weight: bold; margin-left: 10px;">'.esc_html('Custom Post Fields', 'uj-core-functionality').':</legend>
				<div class="inline-edit-col">';
    ...
    add_action('quick_edit_custom_box',  'ujcf_add_quick_edit_order', 10, 2);
}
 ...
 * Save the custom field value from the quick edit box 
 */
if (!function_exists(('ujcf_quick_edit_order_save'))) {
    function ujcf_quick_edit_order_save($post_id)
    {
        if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE)
            return $post_id;
        ...
        if (!empty($_POST['ujcf_nonce']) && !wp_verify_nonce($_POST['ujcf_nonce'], 'ujcf_q_edit_order_nonce')) {
            return;
        }

		$field_name = ujcf_get_quick_edit_field_name(get_post_type($post_id));
		if ($field_name) {
			if (isset($_POST['acf']) && !empty($_POST['acf']['field_' . $field_name])) {
				$value = intval($_POST['acf']['field_' . $field_name]);
			} else {
				$value = 10;
			}
    ...
    add_action('save_post_alert', 'ujcf_quick_edit_order_save');
    add_action('save_post_faq', 'ujcf_quick_edit_order_save');
    add_action('save_post_fundraiser', 'ujcf_quick_edit_order_save');
    add_action('save_post_news', 'ujcf_quick_edit_order_save');
    add_action('save_post_testimonial', 'ujcf_quick_edit_order_save');
}
 ...
 * Populate the custom field values at the quick edit box using Javascript
 */
if (!function_exists('ujcf_quick_edit_order_js')) {
    function ujcf_quick_edit_order_js()
    {
        $current_screen = get_current_screen();
        ...
        ?>
        <!-- add JS script -->
        <script id="ujcf_quick_edit_order_js" type="text/javascript">
            jQuery(function($) {
                var $ujcf_inline_editor = inlineEditPost.edit;
                inlineEditPost.edit = function(id) {
                    $ujcf_inline_editor.apply(this, arguments);
...
<?php
    ...
    add_action('admin_print_footer_scripts-edit.php', 'ujcf_quick_edit_order_js');
}
